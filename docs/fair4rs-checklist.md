...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/white-shark-geolocation-light`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/white-shark-geolocation-light:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/white-shark-geolocation-light:latest`.
